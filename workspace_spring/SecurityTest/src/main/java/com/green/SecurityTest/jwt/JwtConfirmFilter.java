...
public class JwtConfirmFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        //요청 시 헤더에 담겨오는 토큰 확인
        //request 요청에 모든것이 담겨있음 , response 응답에 모든것이 담겨있음
        ...
        //토큰 유효성 검사
        //토큰 추출
        String token = authorization.split(" ")[1]; //aaa bbb ccc "Bearer sdfsdsdssd"

        //1 토큰 만료일 검사
        //토큰의 만료일이 지났을 때 if문 실행
        ...
        //아래의 코드는 토큰이 존재하고 만료기간이 지나지 않았을 때만 실행
        // -> 인증된 회원이다
