...
SELECT 
    ...
    C.CUSTOMER_NAME,
    C.CUSTOMER_ADDR,
    IFNULL(D.DELIVERY_DRIVER_NAME,'-'),
    IFNULL(D.DELIVERY_DRIVER_PHONE,'-'),
    (R.QUANTITY * P.PRODUCT_PRICE) AS TOTALPRICE
FROM 
    orders O
JOIN 
...
LEFT JOIN 
    delivery D ON O.ORDER_NUM = D.ORDER_NUM
WHERE 
...
-- 상품이름,가격, 이미지,내용,수량 ,제조사 ,카테고리
-- 
CREATE TABLE supllies_procurement(
	ITEM_CODE INT AUTO_INCREMENT PRIMARY KEY 
	,ITEM_NAME VARCHAR(50) NOT NULL -- 상품이름
	,ITEM_CONTENT VARCHAR(100) NOT NULL -- 내용
	,ITEM_STOCK INT DEFAULT 20 -- 수량
	,ITEM_STATUS VARCHAR(20) DEFAULT 'FOR_SALE' -- 판매중
	,CATE_CODE INT REFERENCES CATEGORY(CATE_CODE) -- 카테고리
	,ITEM_COMPANY VARCHAR(50) -- 제품회사
);
